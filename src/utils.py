...
def loaddata(input_size, feature):
    ...
    print("LCargando datos de entrenamiento...")
...
class LearningRateSchedulerPerBatch(LearningRateScheduler):
    """ Codigo de https://towardsdatascience.com/resuming-a-training-process-with-keras-3e93152ee11a
    ...
    def __init__(self, schedule, verbose=0):
        ...
        self.count = 0  # Índice de batch global (el argumento batch regular se refiere al índice de batch dentro de la época)
...
def print_results(config, model, Xval, yval, classes):
    ...
    if config.trained_model:
        ...
        model.load_weights('modelos/{}-latest.keras'.format(config.feature))
    # para combinar diferentes modelos entrenados. En pruebas  
    if config.ensemble:
        model2.load_weight('modelos/weights-V1.keras')
...
def add_noise(config):
    ...
    for i, label in enumerate(testlabel):
      if label[1] == '~':
        ...
        noise = np.nan_to_num(noise) # eliminar NaNs e Infs
        from scipy.signal import resample
        noise= resample(noise, int(len(noise) * 360 / 300) ) # remuestrear para que coincida con la frecuencia de muestreo de datos 360(mit), 300(cinc)
        from sklearn import preprocessing
        noise = preprocessing.scale(noise)
        noise = noise/1000*6 # normalizar en bruto, para mejorar
